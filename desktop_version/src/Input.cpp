...
void menuactionpress()
{
	...
	#define NOCUSTOMOFFSET -1
#else
	#define NOCUSTOMOFFSET 0
#endif

#define OFFSET (MPOFFSET + NOCUSTOMOFFSET)
...
void gameinput()
{
	...
	if(!script.running || (script.running && script.passive)) {
		...
		if(key.isDown(KEYBOARD_z) || key.isDown(KEYBOARD_SPACE) || key.isDown(KEYBOARD_v) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_w) || key.isDown(KEYBOARD_s) || key.isDown(game.controllerButton_flip)) {
	...
	if(game.advancetext) {
		if(game.pausescript) {
			game.press_action = false;
			if(key.isDown(KEYBOARD_z) || key.isDown(KEYBOARD_SPACE) || key.isDown(KEYBOARD_v) || key.isDown(KEYBOARD_UP) || key.isDown(KEYBOARD_DOWN) || key.isDown(KEYBOARD_w) || key.isDown(KEYBOARD_s) || key.isDown(game.controllerButton_flip)) game.press_action = true;
		}
...
void mapinput()
{
	...
	if(graphics.menuoffset == 0 && (!game.glitchrunnermode || graphics.fademode == 0) && game.fadetomenudelay <= 0 && game.fadetolabdelay <= 0) {
		if(graphics.flipmode) {
