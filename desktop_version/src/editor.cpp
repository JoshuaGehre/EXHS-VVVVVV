...
int editorclass::absat(int* x, int* y)
{
...
bool editorclass::save(std::string& _path)
{
	...
	for(int i = 0; i < maxwidth * maxheight; i++) {
		...
		// Only save these when needed
		if(level[i].customtileset) {
			edlevelclassElement->SetAttribute("customtileset", level[i].customtileset);
		}
		if(level[i].customspritesheet) {
...
void editorrender()
{
			...
			switch(edentity[i].t) {
				...
				case 11: // Gravity lines
					...
					if(edentity[i].p1 == 0) { //Horizontal
						if(edentity[i].p4 != 1) {
							// Unlocked
							while(tx >= 0 && !ed.spikefree(tx, ey / 8))
								tx--;
							while(tx2 < 40 && !ed.spikefree(tx2, ey / 8))
								tx2++;
							tx++;
							edentity[i].p2 = tx;
						...
						if(edentity[i].p4 != 1) {
							// Unlocked
							while(ty >= 0 && !ed.towerspikefree(tx, ty))
								ty--;
							while(ty2 < y_size && !ed.towerspikefree(tx, ty2))
								ty2++;
							ty++;
							edentity[i].p2 = ty;
				...
				case 50: // Warp lines
					...
					if(edentity[i].p1 >= 2) { //Horizontal
						if(edentity[i].p4 != 1) {
							// Unlocked
							while(tx >= 0 && !ed.free(tx, ey / 8))
								tx--;
							while(tx2 < 40 && !ed.free(tx2, ey / 8))
								tx2++;
							tx++;
							edentity[i].p2 = tx;
							edentity[i].p3 = (tx2 - tx) * 8;
						...
						if(edentity[i].p4 != 1) {
							// Unlocked
							while(ty >= 0 && !ed.towerfree(tx, ty))
								ty--;
							while(ty2 < y_size && !ed.towerfree(tx, ty2))
								ty2++;
							ty++;
							edentity[i].p2 = ty;
							edentity[i].p3 = (ty2 - ty) * 8;
	...
	if(ed.boundarymod > 0) {
		...
		if(ed.level[tmp].enemyx1 != 0 && ed.level[tmp].enemyy1 != 0 && ed.level[tmp].enemyx2 != 320 && ed.level[tmp].enemyy2 != 240) {
		...
		if(ed.level[tmp].platx1 != 0 && ed.level[tmp].platy1 != 0 && ed.level[tmp].platx2 != 320 && ed.level[tmp].platy2 != 240) {
	...
	if(game.ghostsenabled) {
		for(int i = 0; i < (int) ed.ghosts.size(); i++) {
			if(i <= ed.currentghosts) { // We don't want all of them to show up at once :)
				if(ed.ghosts[i].rx != ed.levx || ed.ghosts[i].ry != ed.levy || !INBOUNDS(ed.ghosts[i].frame, graphics.sprites))
					continue;
		...
		switch(ed.drawmode) {
			...
			case 18: //Coins can be multiple sizes
				if(ed.zmod)
					...
					fillboxabs((ed.tilex * 8), (ed.tiley * 8), 24, 24, graphics.getRGB(200, 32, 32));
				else if(ed.vmod)
					fillboxabs((ed.tilex * 8), (ed.tiley * 8), 24, 24, graphics.getRGB(200, 32, 32));
				else
					fillboxabs((ed.tilex * 8), (ed.tiley * 8), 8, 8, graphics.getRGB(200, 32, 32));
	...
	if(!ed.settingsmod && !ed.scripteditmod) {
		...
		switch(ed.drawmode) {
			...
			case 18:
				if(ed.zmod)
					graphics.bprint(2, 2, "^2+Z: 10 Coin", 196, 196, 255 - help.glow);
				else if(ed.xmod)
					graphics.bprint(2, 2, "^2+X: 20 Coin", 196, 196, 255 - help.glow);
				else if(ed.cmod)
					graphics.bprint(2, 2, "^2+C: 50 Coin", 196, 196, 255 - help.glow);
				else if(ed.vmod)
					graphics.bprint(2, 2, "^2+V: 100 Coin", 196, 196, 255 - help.glow);
				else
...
void editorinput()
{
	...
	if(ed.scripteditmod) {
		if(ed.scripthelppage == 0) {
			...
			if((up_pressed || key.keymap[SDLK_KP_8]) && ed.keydelay <= 0) {
				ed.keydelay = 6;
				ed.hookmenu--;
			}

			if((down_pressed || key.keymap[SDLK_KP_2]) && ed.keydelay <= 0) {
				ed.keydelay = 6;
				ed.hookmenu++;
			}
			...
			if(!game.press_action && !game.press_left && !game.press_right && !up_pressed && !down_pressed && !key.isDown(27)) game.jumpheld = false;
			if(!game.jumpheld) {
				if(game.press_action || game.press_left || game.press_right || game.press_map || up_pressed || down_pressed || key.keymap[SDLK_KP_8] || key.keymap[SDLK_KP_2] || key.isDown(27)) {
					game.jumpheld = true;
				}
		...
		*ed.textptr = key.keybuffer;
		...
		if(ed.settingsmod) {
			if(!game.press_action && !game.press_left && !game.press_right && !up_pressed && !down_pressed) game.jumpheld = false;
			if(!game.jumpheld) {
				...
				if(ed.trialmod) {
					...
					if(game.currentmenuoption == 3) {
						if(game.press_left || up_pressed) ed.customtrials[ed.edtrial].trinkets--;
						if(game.press_right || down_pressed) ed.customtrials[ed.edtrial].trinkets++;
					...
					if(game.currentmenuoption == 4) {
						if(game.press_left || up_pressed) ed.customtrials[ed.edtrial].par--;
						if(game.press_right || down_pressed) ed.customtrials[ed.edtrial].par++;
			...
			if(up_pressed || down_pressed ||
			   ...
			   key.keymap[SDLK_KP_4] || key.keymap[SDLK_KP_6]) {
				ed.keydelay = 6;
				if(up_pressed || key.keymap[SDLK_KP_8])
					ed.mapheight--;
				else if(down_pressed || key.keymap[SDLK_KP_2])
					ed.mapheight++;
				else if(left_pressed || key.keymap[SDLK_KP_4])
					ed.mapwidth--;
				else if(right_pressed || key.keymap[SDLK_KP_6])
					ed.mapwidth++;
				...
				if(key.leftbutton) {
					if(ed.lclickdelay == 0) {
						...
						if(tmp == -1 && ed.free(ed.tilex, ed.tiley) == 0) {
							...
							{
								if(ed.zmod)
									addedentity(tx, ty, 8, 1);
								else if(ed.xmod)
									addedentity(tx, ty, 8, 2);
								else if(ed.cmod)
									addedentity(tx, ty, 8, 3);
								else if(ed.vmod)
									addedentity(tx, ty, 8, 4);
								else
	...
	for(size_t i = 0; i < altstates.size(); i++)
		if(altstates[i].x == -1 || altstates[i].y == -1) {
			...
			for(int i = 0; i < limit; i++)
				if(edentity[i].x >= rxi * 40 && edentity[i].x < (rxi + 1) * 40 && edentity[i].y >= ryi * 30 && edentity[i].y < (ryi + 1) * 30 && edentity[i].state == 0 && edentity[i].intower == 0) {
					if(edentity[i].t == 9) {
...
void editorclass::removealtstate(int rxi, int ryi, int state)
{
	...
	for(size_t i = 0; i < edentity.size(); i++)
		if(edentity[i].x >= rxi * 40 && edentity[i].x < (rxi + 1) * 40 && edentity[i].y >= ryi * 30 && edentity[i].y < (ryi + 1) * 30 && edentity[i].state == state && edentity[i].intower == 0) {
			removeedentity(i);
		}
...
void editorclass::switchroomsinput()
{
	if(up_pressed || down_pressed ||
	   ...
	   key.isDown(SDL_CONTROLLER_BUTTON_DPAD_LEFT) || key.isDown(SDL_CONTROLLER_BUTTON_DPAD_RIGHT)) {
		ed.keydelay = 6;
		if(up_pressed || key.keymap[SDLK_KP_8] || key.isDown(SDL_CONTROLLER_BUTTON_DPAD_UP))
			ed.levy--;
		else if(down_pressed || key.keymap[SDLK_KP_2] || key.isDown(SDL_CONTROLLER_BUTTON_DPAD_DOWN))
			ed.levy++;
		else if(left_pressed || key.keymap[SDLK_KP_4] || key.isDown(SDL_CONTROLLER_BUTTON_DPAD_LEFT))
			ed.levx--;
		else if(right_pressed || key.keymap[SDLK_KP_6] || key.isDown(SDL_CONTROLLER_BUTTON_DPAD_RIGHT))
			ed.levx++;
		ed.updatetiles = true;
		ed.changeroom = true;
		graphics.backgrounddrawn = false;
	...
	for(size_t i = 0; i < edentity.size(); i++) {
		if(edentity[i].t == 8) {
			if(edentity[i].p1 == 0)
				...
				temp += 10;
			else if(edentity[i].p1 == 2)
				temp += 20;
			else if(edentity[i].p1 == 3)
				temp += 50;
			else if(edentity[i].p1 == 4)
				temp += 100;
		}
	}
	return temp;
