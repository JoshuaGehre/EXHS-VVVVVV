...
std::vector<SDL_Surface*>* Graphics::selectspritesheet()
{
	int customs = ed.getcustomsprites();
	if((customs > 1) && (customsprites.find(customs) != customsprites.end())) {
		return &customsprites[customs];
	} else {
		return flipmode ? (&flipsprites) : (&sprites);
	}
}
...
void Graphics::drawspritesetcol(int x, int y, int t, int c, int flipped /*= 0*/)
{
	...
	BlitSurfaceColoured((*selectspritesheet())[t], NULL, backBuffer, &rect, ct, flipped);
}
...
#define PROCESS_TILESHEET_CHECK_ERROR(tilesheet, tile_square)                                      \
	if(grphx.im_##tilesheet->w % tile_square != 0 || grphx.im_##tilesheet->h % tile_square != 0) { \
...
void Graphics::drawsprite(int x, int y, int t, int r, int g, int b)
{
	...
	BlitSurfaceColoured((*selectspritesheet())[t], NULL, backBuffer, &rect, ct);
}
...
void Graphics::drawtile(int x, int y, int t)
{
	...
	if(INBOUNDS(t, tiles) && t >= 14 && t <= 17 && customts <= 3 && (!FILESYSTEM_assetsmounted || ed.onewaycol_override)) {
...
void Graphics::drawtile2(int x, int y, int t)
{
	int customts = ed.getcustomtiles();
	SDL_Rect rect = {Sint16(x), Sint16(y), tiles_rect.w, tiles_rect.h};
	if(INBOUNDS(t, tiles2) && t >= 14 && t <= 17 && customts <= 3 && (!FILESYSTEM_assetsmounted || ed.onewaycol_override)) {
		colourTransform thect = {ed.getonewaycol()};
	...
	for(int i = obj.entities.size() - 1; i >= 0; i--) {
		...
		switch(obj.entities[i].size) {
			case 0: {
				...
				//screenwrapping!
				if(map.warpx ||
				   (map.towermode && !map.minitowermode && map.ypos >= 500 && map.ypos <= 5000)) //The "wrapping" area of the tower
				{
					if(tpoint.x < 0) {
						tpoint.x += 320;
						drawRect = sprites_rect;
...
void Graphics::drawforetile(int x, int y, int t)
{
	...
	if(!noclear || (tile >= 6 && tile <= 9) || tile == 49 || tile == 50 || tile == 1 || (tile >= 80 && tile <= 679)) {
		...
		setRect(rect, x, y, tiles_rect.w, tiles_rect.h);
		int customts = ed.getcustomtiles();
		if(INBOUNDS(tile, tiles) && tile >= 14 && tile <= 17 && customts <= 3 && (!FILESYSTEM_assetsmounted || ed.onewaycol_override)) {
			colourTransform thect = {ed.getonewaycol()};
...
void Graphics::drawforetile2(int x, int y, int t)
{
	...
	if(!noclear || (tile >= 6 && tile <= 9) || tile == 49 || tile == 50 || tile == 1 || (tile >= 80 && tile <= 679)) {
		...
		setRect(rect, x, y, tiles_rect.w, tiles_rect.h);
		int customts = ed.getcustomtiles();
		if(INBOUNDS(tile, tiles2) && tile >= 14 && tile <= 17 && customts <= 3 && (!FILESYSTEM_assetsmounted || ed.onewaycol_override)) {
			colourTransform thect = {ed.getonewaycol()};
...
void Graphics::reloadresources(bool fast /*= false*/)
{
	...
	for(auto x : customtiles) {
		for(auto y : x.second) {
			SDL_FreeSurface(y);
		}
	}
	customtiles.clear();
	for(auto x : customsprites) {
		for(auto y : x.second) {
			SDL_FreeSurface(y);
		}
	}
