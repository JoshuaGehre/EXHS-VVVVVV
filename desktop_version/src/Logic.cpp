	...
	if(game.deathseq != -1) {
		...
		if(map.towermode) {
			...
			if(obj.customwarpmode) {
				...
				obj.customwarpmodevon = false;
				int player = obj.getplayer();
				if(player > -1 && ((game.door_left > -2 && obj.entities[player].xp < -14 + 16) || (game.door_right > -2 && obj.entities[player].xp >= 308 - 16)))
		...
		if(map.towermode && map.minitowermode) {
			...
			if(map.warpy)
				dowrap = true;
			else if(player > -1 && ((game.door_left > -2 && obj.entities[player].xp < -14) || (game.door_right > -2 && obj.entities[player].xp >= 308))) {
